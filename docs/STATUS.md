...
- **The 25 proxied and licensed translations.** They reach API.Bible or Tyndale
  through our backend, which does not exist yet. The two Twi Bibles used to be
  among them and are now bundled instead, so they need neither.
...
**Build 20** (version 1.0.0) is the current one. Built locally, signed, and
verified by unpacking the .ipa and reading `CFBundleVersion` out of it rather
than trusting that the build succeeded. Build 19 is on TestFlight; 13 to 18
came before it.

Every build so far was made against the placeholder backend URL — `eas.json`
still points the production profile at `https://CHANGE-ME.example.com`, which
does not resolve, so server calls fail at the network layer and screens fall
back to their cached-or-empty states rather than crashing.

**What build 20 exercises honestly** (no server, no account): the Bible reader
and search over the 31,102-verse KJV, **both Twi Bibles**, verse of the day and
its rotation, copying a verse, highlights and reading position, the quiz across
all 66 books with the verse behind each answer, all 35 reading plans including
the scripture for each day and the version picker over it, on-device notes and
prayers, and note export to PDF.

**What will fail in it**: sign-in, friends and nudges, the blog, notes and
prayer sync, streaks, reflections, account deletion, the welcome email, sharing
a plan by code, and the 25 translations that route through the server. All
present in the binary, all waiting on a backend.
