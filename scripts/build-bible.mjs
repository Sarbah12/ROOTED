...
import { writeOfflineIndex } from './build-offline-index.mjs';
...
`;
  ...
  // The loader index covers every bundled translation, not just the KJV, so
  // it is generated in one place — rebuilding the KJV must not drop the Twi
  // editions that scripts/import-vpl-bible.mjs put there.
  const bundled = await writeOfflineIndex();
  ...
  console.log(`Wrote constants/bible-offline.ts — ${bundled.join(', ')}`);
}
